...
import os
from classifier_models import SimpleDLA
...
def ratio_test(path="/scratch/private/eunbiyoon/sub_Levy/2MNISTbatch128lr0.0001ch128ch_mult[1, 2, 2, 2]num_res2dropout0.1clamp201.8_0.1_20/ckpt/MNISTbatch128ch128ch_mult[1, 2, 2, 2]num_res2dropout0.1clamp20_epoch1248_1.8_0.1_20.pth",
               alpha=1.8, datasets='MNIST', device='cuda',max_iter =1,conditional=True, num_classes=10,
               ch=128, ch_mult=[1,2,2,2], num_res_blocks=2, resolution=28, name='', y= None):
    ...
    if datasets == "CIFAR10":
        model = SimpleDLA()
        model = net.to(device)
        ckpt = torch.load("/scratch/private/eunbiyoon/pytorch-cifar/checkpoint/ckpt.pth",
                      map_location=device)
        num_classes = 10
    elif datasets == "MNIST":
        model = Net(datasets).to(device)
